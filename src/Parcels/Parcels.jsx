...
const stripSpaces = (s) => String(s).replace(/\s+/g, ""); // в API без пробелов
const sanitizeTN  = (s) => norm(String(s));               // в UI с пробелами
const TN_MIN = 1;
const TN_MAX = 32;
const LS_TRACKS = "lc_tracks";

/** ===== задержки синтетических событий =====
 * Для проверки можно временно поставить:
 *   const TRUCK_DELAY_MS = 60 * 1000; // 1 минута
 */
...
/** карточки-счётчики */
const STAGES = [
  { key: "accepted", label: "Принят на склад в Китае" },
  { key: "sent",     label: "Отправлен из Китая" },
  { key: "arrived",  label: "Прибыл в пункт выдачи" },
  { key: "done",     label: "Получен" },
];
...
function renderSmartStatus(status) {
  ...
  const m = text.match(/^(.*?)(?:\s*\[([^]*?)\])\s*$/); // before [inside]
  if (!m) return text;

  const before = m[1].trim();
  const inside = m[2].trim();

  // перенос перед известными метками; адрес (с запятыми внутри) идёт целиком
  let parts = inside
    .replace(/\s*,\s*(?=(трек-номер|адрес|телефон|индекс|получатель)\s*:)/gi, "\u0000")
    .split("\u0000");

  // fallback на 3 части, если меток нет
  if (parts.length === 1) {
...
const TrackInput = ({ value, onChange, suggestionsPool }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const hints = list.map((x) => (typeof x === "string" ? x : x?.tracking_number)).filter(Boolean);
  ...
  return (
    <div className="parcels__track-row" ref={wrapRef}>
      <label htmlFor={inputId} className="parcels__visually-hidden">Трек-номер</label>
      <input
        id={inputId}
...
function withSyntheticEvents(events) {
  ...
  // helper: найти ПОСЛЕДНИЙ индекс по предикату
  const findLastIdx = (arr, pred) => {
    for (let i = arr.length - 1; i >= 0; i--) if (pred(arr[i], i)) return i;
    return -1;
  };

  // 1) «На хранение» (+10s) — только если прошло время
  ...
  // 2) «Грузовик в пути» (+1 день) — только если прошло время
...
const Modal = ({ open, title, loading, error, items, onClose }) => {
  ...
  useEffect(() => { setDetail(null); setDError(""); }, [open, items]);
  ...
  const openDetail = async (tnApi, tnDisplay) => {
    ...
    } catch { setDError("История не найдена."); }
      ...
      <div className="modal__dialog">
        <div className="modal__header">
          ...
          <div className="modal__actions">
            {detail && (
              <button type="button" className="modal__action" onClick={() => setDetail(null)} aria-label="Назад">←</button>
            )}
        ...
        <div className="modal__body">
          {!detail && (
            <>
              {loading && <div className="modal__muted">Загрузка…</div>}
              ...
              {!loading && !error && items.length > 0 && (
                <div className="modal__list">
                  {items.map((o) => {
                    ...
                    const onClick = () => openDetail(tn, tn);
                    return (
                      <button type="button" key={o.id ?? tn} className="modalcard" onClick={onClick} aria-label={`Открыть историю трека ${tn}`}>
          ...
          {detail && (
            <>
              {dLoading && <div className="modal__muted">Загрузка…</div>}
              {dError && <div className="modal__error">{dError}</div>}
              {!dLoading && !dError && <Timeline order={detail} />}
            </>
          )}
        </div>
      </div>
...
/* ===== Stage card ===== */
const StageCard = ({ label, count, onClick, hideCount = false }) => (
  <button type="button" className="parcels__stage" aria-label={label} onClick={onClick}>
    <div className="parcels__stage-label">{label}</div>
...
const BucketList = ({ title, items }) => {
  ...
  return (
    <div className="parcels__bucket">
      <div className="parcels__bucket-title">{title} — {items.length}</div>
      <div className="parcels__bucket-list">
        {items.map((t) => <span className="parcels__tag" key={t}>{t}</span>)}
      </div>
    </div>
  );
};
...
const Parcels = () => {
  ...
  const [bucket, setBucket] = useState({ accepted: [], sent: [], arrived: [], done: [] });
  ...
  const fetchCounters = async () => {
    ...
    try {
      ...
      if (code === 401 || code === 403) setError("Требуется вход в аккаунт, чтобы увидеть ваши заказы.");
    }
  };
  ...
  const openTrackModal = async (tnApi, tnDisplay) => {
    ...
    setMItems([]); setMError(""); setMLoading(true); setMOpen(true);
    try {
      const { data } = await api.get(TRACK_URL(tnApi));
      const order = data?.order || data;
  ...
  useEffect(() => {
    ...
    if (tn) {
      ...
      setTracks([tn, ""]);
      openTrackModal(stripSpaces(tn), tn); // API — без пробелов, UI — как ввёл
    }
  }, []);
  ...
  const setTrack = (i, v) =>
    setTracks((arr) => {
      ...
      if (i === arr.length - 1 && norm(v)) next.push("");
  ...
  const claimOrCreate = async (tnClean) => {
    try {
      ...
      try {
        ...
        return { ok: true, created: true, data: res2.data };
      } catch {
  ...
  const submit = async (ev) => {
    ...
    // пары { raw (UI), clean (API) }
    const prepared = uniq(tracks.map((t) => sanitizeTN(t))).filter(Boolean)
      .map((raw) => ({ raw, clean: stripSpaces(raw) }));

    if (prepared.length === 0) { setError("Введите трек-номер."); return; }

    const bad = prepared.map(({ raw, clean }) => ({ raw, err: validateTN(clean) })).filter((x) => x.err);
    if (bad.length) { setError(`${bad[0].raw}: ${bad[0].err}`); return; }
    ...
    for (const p of prepared) {
      const res = await claimOrCreate(p.clean);
      if (res.ok) ok.push({ ...p, data: res.data });
      else hasFails = true;
    }
    ...
    if (ok.length) {
      try {
        const raw = localStorage.getItem(LS_TRACKS);
        const arr = Array.isArray(JSON.parse(raw)) ? JSON.parse(raw) : [];
        const next = uniq([...ok.map(x => x.raw), ...arr]).slice(0, 200);
        localStorage.setItem(LS_TRACKS, JSON.stringify(next));
        setHistory(next);
      } catch {}
    }

    const group = { accepted: [], sent: [], arrived: [], done: [] };
    for (const { raw, data } of ok) {
      const order = data?.order || data;
      const createdEvent = data?.created_event;
      const statusStr = String(order?.last_status || order?.status || createdEvent?.status || "");
      const key = extractStatus({ last_status: statusStr, events: order?.events || [] });
      if (key) group[key].push(raw);
    }
    setBucket(group);
    ...
    if (ok.length === 0) {
      ...
      setMessage(`Добавлено в ваш аккаунт: ${ok.length}.`);
      if (hasFails) setError("Часть треков не удалось обработать (возможно, уже принадлежит другому аккаунту).");
    }

    if (ok.length === 1 && prepared.length === 1) openTrackModal(ok[0].clean, ok[0].raw);
  ...
  return (
    <div className="parcels">
      <div className="parcels__card">
        <div className="parcels__logo"><img src={logo} alt="Lider Cargo" /></div>

        <form className="parcels__block" onSubmit={submit} noValidate>
          <div className="parcels__block-title">Добавить трек-номер</div>

          <div className="parcels__inputs" aria-live="polite">
            {tracks.map((t, i) => (
              <TrackInput
                key={i}
                value={t}
                onChange={(v) => setTrack(i, v)}
                suggestionsPool={history}
              />
            ))}
          </div>
          ...
          <div className="parcels__hint">Длина трек-номера {TN_MIN}–{TN_MAX} символов.</div>
          {message && <div className="parcels__success">{message}</div>}
          {error && <div className="parcels__error">{error}</div>}
        </form>

        <div className="parcels__grid" aria-live="polite">
          <StageCard label={STAGES[0].label} count={globalCounts.accepted} onClick={() => openStageModal("accepted")} />
          <StageCard label={STAGES[1].label} count={globalCounts.sent}     onClick={() => openStageModal("sent")} />
          <StageCard label={STAGES[2].label} count={globalCounts.arrived}  onClick={() => openStageModal("arrived")} />
          <StageCard label={STAGES[3].label} onClick={() => openStageModal("done")} hideCount />
        </div>

        {(bucket.accepted.length + bucket.sent.length + bucket.arrived.length + bucket.done.length > 0) && (
          <div className="parcels__results">
            <div className="parcels__results-title">Результаты добавления</div>
            <BucketList title={STAGES[0].label} items={bucket.accepted} />
            <BucketList title={STAGES[1].label} items={bucket.sent} />
            <BucketList title={STAGES[2].label} items={bucket.arrived} />
            <BucketList title={STAGES[3].label} items={bucket.done} />
          </div>
        )}
      </div>
