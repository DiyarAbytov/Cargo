...
const SiteShell = ({ children }) => {
  ...
  useEffect(() => {
    // Запрос к API для получения URL фона
    api.get("/base/")
      .then((response) => {
        const bannerUrl = response.data.results[0]?.banner || "";
        // Декодируем URL
        const decodedUrl = decodeURIComponent(bannerUrl);
        setBackgroundImageUrl(decodedUrl); // Устанавливаем декодированный URL
      })
      .catch((error) => {
        console.error("Ошибка получения данных фона", error);
      });
  }, []);

  return (
    <div
      className={`site ${showHeader ? "" : "site--no-header"}`}
      style={{
        backgroundImage: `url(${backgroundImageUrl})`,  // Применяем фоновое изображение
