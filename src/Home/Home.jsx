...
/* helpers */
const extractShortCode = (display) => {
  const m = String(display || "").match(/(\d{2}-\d{2}\([^)]+\))/);
  return m ? m[1] : "";
};
const splitDisplayCode = (display) => {
  const m = String(display || "").match(/^(.+?)\s*\(([^)]+)\)\s*$/);
  return m ? { top: norm(m[1]), bottom: norm(m[2]) } : { top: String(display || ""), bottom: "" };
};
...
/* ===== синтетика событий — только ПОСЛЕ наступления времени ===== */
const normStatusText = (t) => String(t || "").toLowerCase().replace(/\s+/g, " ").trim();
const findLastIdx = (arr, pred) => { for (let i = arr.length - 1; i >= 0; i--) if (pred(arr[i], i)) return i; return -1; };
...
function renderSmartStatus(status) {
  ...
  // Разбиваем только перед известными метками, адрес остаётся целым (со своими запятыми)
  let parts = inside
    .replace(/\s*,\s*(?=(трек-номер|адрес|телефон|индекс|получатель)\s*:)/gi, "\u0000")
    .split("\u0000");

  // fallback: первая часть + вторая + остальное
  if (parts.length === 1) {
...
const Home = () => {
  ...
  const [codeBottom, setCodeBottom] = useState("");
  ...
  const [events, setEvents] = useState([]); // [{status, location, timestamp}]
  ...
  // мигание/изменение кнопок при копировании
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setClientCodeShort(extractShortCode(displayCode));
        const { top, bottom } = splitDisplayCode(displayCode);
        setCodeTop(top);
        setCodeBottom(bottom);
  ...
  useEffect(() => {
    ...
    const onEsc = (e) => { if (e.key === "Escape") { setOpen(false); setAddrOpen(false); setTrackOpen(false); } };
  ...
  const loadTrack = async (t) => {
    ...
    try {
      ...
      setEvents(normalizeEvents(data)); // синтетику добавим ниже при рендере
    } catch (e) {
      if (e?.response?.status === 401) {
        authStore.clear();
        window.location.replace("/login");
  ...
  const goCheck = (e) => {
    ...
    setError(err);
    if (err) { inputRef.current?.focus(); return; }
  ...
  // копирование + тосты
  const clipClientShort = async () => {
    try {
      await navigator.clipboard.writeText(clientCodeShort || "");
      showToast("Короткий код скопирован");
      flash(setCopiedCode, codeBlink);
    } catch {}
  };
  const clipAddressLine  = async () => {
    const line = joinOneLine(cnAddress, clientCodeShort, cnContact, cnPhone);
    try {
  ...
  /* применяем синтетику и строим таймлайн */
  const enrichedAsc = useMemo(() => withSyntheticEventsAscending(events), [events]); // по возрастанию
  const timeline = useMemo(() => [...enrichedAsc].reverse(), [enrichedAsc]); // новый → старый
    ...
    <div className="home">
      <div className="home__card">
        ...
        <div className="home__head">
          <div className="home__code">
            <div className="home__sub">Ваш клиентский код</div>
            <div className="home__chip">
              <div className="home__chip-lines">
                <span className="home__chip-top">{codeTop || "—"}</span>
                {codeBottom ? <span className="home__chip-btm">({codeBottom})</span> : null}
              </div>
              <button
                className={`home__chip-btn ${copiedCode ? "is-copied" : ""}`}
                type="button"
                onClick={clipClientShort}
                aria-label="Скопировать короткий код"
                title="Скопировать короткий код"
              >
                <CopyIco/>
              </button>
            </div>
          </div>
          <img src={logo} alt="Lider Cargo" className="home__brand" />
        </div>

        {/* поиск */}
        <form className="home__track" onSubmit={goCheck} autoComplete="off" ref={wrapRef}>
          <div className="home__label">Отследить товар</div>
          <div className="home__track-wrap">
            <label className="home__visually-hidden" htmlFor="home-track">Трек-номер</label>
            <input
              id="home-track"
              ref={inputRef}
              ...
              value={track}
              onChange={(e) => { const v = clearSpaces(e.target.value); setTrack(v); if (error) setError(""); }}
              onFocus={() => setOpen(true)}
              maxLength={TN_MAX}
              aria-invalid={!!error}
              aria-describedby={error ? "home-track-err" : undefined}
              onKeyDown={(e) => { if (e.key === "Enter") goCheck(e); }}
              inputMode="text"
              autoCapitalize="off"
              autoCorrect="off"
              spellCheck={false}
            />
            <button className="home__search-ico" type="submit" aria-label="Проверить"><SearchIco/></button>

            {open && (
              <div className="home__menu" role="listbox" onMouseLeave={() => setOpen(false)}>
                <div className="home__list" onScroll={onScroll}>
                  {shown.length === 0 ? <div className="home__empty">Нет подсказок</div> : shown.map((s, i) => (
                    <div
                      key={`${s}-${i}`}
          ...
          <div className="home__hint">Длина трек-номера {TN_MIN}–{TN_MAX} символов, без пробелов.</div>
          {error ? <div id="home-track-err" className="home__error" role="alert" aria-live="polite">{error}</div> : null}
        </form>

        {/* карточка адреса */}
        <div className="home__label">Адрес</div>
        {loading ? (
          <div className="home__loading">Загрузка…</div>
        ) : (
          <div className="home__addr-card">
            <div className="home__addr-row">
              <div className="home__addr-key">Адрес склада в Китае</div>
              <div className="home__addr-val">
                <button type="button" className="home__addr-eye" onClick={() => setAddrOpen(true)} aria-label="Показать адрес полностью">
                  <EyeIco/>
                </button>
                <div className="home__addr-text">{cnAddress || "—"}</div>
              </div>
            </div>

            <div className="home__addr-row">
              <div className="home__addr-key">Пункты самовывоза</div>
              <div className="home__addr-val">
                <div className="home__addr-text">{pickupText || "—"}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {/* копирование строки */}
        <button className={`home__copy ${copiedAddrBtn ? "is-copied" : ""}`} type="button" onClick={clipAddressLine}>
      ...
      {addrOpen && (
        <div className="home__modal" role="dialog" aria-modal="true" onClick={() => setAddrOpen(false)}>
          <div className="home__modal-card" onClick={(e) => e.stopPropagation()}>
            <div className="home__modal-head">
              <div className="home__modal-title">Адрес склада</div>
              <button className="home__close" type="button" onClick={() => setAddrOpen(false)} aria-label="Закрыть">
                <XIco/>
              </button>
            </div>
            <div className="home__modal-body">
              <div className="home__pair"><div className="home__pair-key">Адрес (CN)</div><div className="home__pair-val">{cnAddress || "—"}</div></div>
              <div className="home__pair"><div className="home__pair-key">Контакт (CN)</div><div className="home__pair-val">{cnContact || "—"}</div></div>
              <div className="home__pair"><div className="home__pair-key">Телефон (CN)</div><div className="home__pair-val">{cnPhone || "—"}</div></div>
            </div>
            <div className="home__modal-actions">
              <button className={`home__copy ${copiedModalAddrBtn ? "is-copied" : ""}`} type="button" onClick={clipAddressOnly}>
      ...
      {trackOpen && (
        <div className="home__modal" role="dialog" aria-modal="true" onClick={() => setTrackOpen(false)}>
          <div className="home__modal-card" onClick={(e) => e.stopPropagation()}>
            <div className="home__modal-head">
              <div className="home__modal-title">Статусы по треку</div>
            ...
            <div className="home__modal-body">
              <div className="home__pair" style={{marginBottom:4}}>
                <div className="home__pair-key">Трек-номер</div>
                <div className="home__pair-val">{selectedTrack}</div>
              </div>

              {trackLoading && <div className="home__loading" style={{marginTop: 8}}>Загрузка статусов…</div>}
                  ...
                  ? (
                    <>
                      ...
                      <div className="home__pair" style={{marginTop: 8}}>
                        <div className="home__pair-key">Текущий статус</div>
                        <div className="home__pair-val">
                            ...
                            <div className="timeline__content">
                              ...
                              {(ev.location || ev.timestamp) && (
                                <div className="timeline__meta">
                                  {ev.location && <span className="timeline__loc">{ev.location}</span>}
                                  {ev.timestamp && <span className="timeline__time">{fmtTsSec(ev.timestamp)}</span>}
                                </div>
                              )}
                            </div>
                          </div>
            ...
            <div className="home__modal-actions">
              <button className="home__copy" type="button" onClick={() => setTrackOpen(false)}>
                Закрыть
              </button>
            </div>
          </div>
      ...
      {/* тост */}
      <div className={`home__toast ${toast ? "is-show" : ""}`} role="status" aria-live="polite">{toast || ""}</div>
    </div>
  );
};
